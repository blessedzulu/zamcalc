...
<header x-data="{ open: false }" x-on:keydown.window.escape="open = false"
  ...
  <nav class="mx-auto flex items-center justify-between px-6 py-3 text-base-100 lg:px-8" aria-label="global">
    <div class="flex items-center pr-7">
      <x-app.logo />
      <div class="flex gap-4 ml-8">
        <x-ui.link :href="route('dashboard')" :attributes="$attributes->when(
            Request::routeIs('dashboard'),
            fn($attr) => $attr->class('dark:!text-white text-neutral-900'),
        )" variant="soft">
          dashboard
        </x-ui.link>

        <x-ui.link :href="route('settings.account')" :attributes="$attributes->when(
  ...
  <!-- Mobile Menu -->
  <div x-show="open" class="lg:hidden" x-ref="dialog" x-cloak="" aria-modal="true">
    ...
    <div
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center">
          <button type="button" class="-m-2.5 rounded-md p-2.5 text-base-100" x-on:click="open = false">
